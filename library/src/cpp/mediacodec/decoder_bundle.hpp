...
public:
    ...
    static decoder_bundle_result create(std::string_view filename, int target_sample_rate = 44100);

    /// Try to create a decoder_bundle from an internal_asset
    /// @param asset Valid internal_asset object
    /// @return Result with either a @p decoder_bundle, or delegated @p decoder_bundle_error
    static decoder_bundle_result create(internal_asset &asset, int target_sample_rate = 44100);
